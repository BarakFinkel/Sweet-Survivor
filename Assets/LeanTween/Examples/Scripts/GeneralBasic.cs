...
public class GeneralBasic : MonoBehaviour {
	...
	public UnityEngine.GameObject prefabAvatar;

	void Start () {
        // Setup
        UnityEngine.GameObject avatarRotate = UnityEngine.GameObject.Find("AvatarRotate");
        UnityEngine.GameObject avatarScale = UnityEngine.GameObject.Find("AvatarScale");
        UnityEngine.GameObject avatarMove = UnityEngine.GameObject.Find("AvatarMove");
	...
	void advancedExamples(){
        LeanTween.delayedCall(gameObject, 14f, ()=>{
			for(int i=0; i < 10; i++){
                ...
                UnityEngine.GameObject rotator = new UnityEngine.GameObject("rotator"+ i);
				rotator.transform.position = new Vector3(10.2f,2.85f,0f);

                // Instantiate Avatar
                UnityEngine.GameObject dude = (UnityEngine.GameObject)UnityEngine.GameObject.Instantiate(prefabAvatar, Vector3.zero, prefabAvatar.transform.rotation );
