...
public class GeneralCameraShake : MonoBehaviour {
	...
	private UnityEngine.GameObject avatarBig;
	private float jumpIter = 9.5f;
	private AudioClip boomAudioClip;

	// Use this for initialization
	void Start () {
        avatarBig = UnityEngine.GameObject.Find("AvatarBig");
        ...
        LeanTween.moveY(avatarBig, height, 1f).setEase(LeanTweenType.easeInOutQuad).setOnComplete( ()=>{
            LeanTween.moveY(avatarBig, 0f, 0.27f).setEase(LeanTweenType.easeInQuad).setOnComplete( ()=>{
				...
				********************/
                ...
                UnityEngine.GameObject[] boxes = UnityEngine.GameObject.FindGameObjectsWithTag("Respawn"); // I just arbitrarily tagged the boxes with this since it was available in the scene
		        foreach (UnityEngine.GameObject box in boxes) {
		            box.GetComponent<Rigidbody>().AddForce(Vector3.up * 100 * height);
		        }

                // Make the lamps spin from the big stomping
                UnityEngine.GameObject[] lamps = UnityEngine.GameObject.FindGameObjectsWithTag("GameController"); // I just arbitrarily tagged the lamps with this since it was available in the scene
		        foreach (UnityEngine.GameObject lamp in lamps) {
